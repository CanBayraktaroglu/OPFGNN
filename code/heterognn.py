...
class ACOPFGNN(torch.nn.Module):
    def __init__(self, hidden_channels: int, out_channels: int, num_layers: int, dropout: float, act_fn: str = "elu"
        ...
        for _ in range(self.num_layers):
            conv = ModuleDict({
                'SB__isConnected__PQ': TransformerConv(-1, self.hidden_channels, edge_dim=2,
                                                                   heads=self.heads), #TransformerConv(-1, hidden_channels, edge_dim=2, heads=self.heads),
            ...
            fc = ModuleDict({
                ...
                'SB__isConnected__PQ': Linear(-1, self.out_channels, bias=True),

                'PV__isConnected__PQ': Linear(-1, self.out_channels, bias=True),

                'NB__isConnected__PQ': Linear(-1, self.out_channels, bias=True),

                'PQ__isConnected__NB': Linear(-1, self.out_channels, bias=True),

                'PQ__isConnected__SB': Linear(-1, self.out_channels, bias=True),

                'PQ__isConnected__PV': Linear(-1, self.out_channels, bias=True),
                'PV__isConnected__NB': Linear(-1, self.out_channels, bias=True),
                'NB__isConnected__PV': Linear(-1, self.out_channels, bias=True),

                'PV__isConnected__PV': Linear(-1, self.out_channels, bias=True),

                'PQ__isConnected__PQ': Linear(-1, self.out_channels, bias=True),
                'NB__isConnected__NB': Linear(-1, self.out_channels, bias=True),
        ...
        for _ in range(4):
            fc = ModuleDict({
                ...
                'SB': Linear(-1, self.out_channels, bias=True),

                'PQ': Linear(-1, self.out_channels, bias=True),

                'PV': Linear(-1, self.out_channels, bias=True),

                'NB': Linear(-1, self.out_channels, bias=True)

            })
            self.fcs.append(fc)

    def lazy_init(self, lazy_input, net, index_mappers):
        x_dict, constraint_dict, edge_idx_dict, edge_attr_dict, bus_idx_neighbors_dict, _, _, _ = hp.extract_unsupervised_inputs(lazy_input, net, index_mappers)
        self.init_layer_norm(x_dict)
        output = self.forward(x_dict, edge_idx_dict, edge_attr_dict, check_nans=True)
        print(output)
    ...
    def forward(self, x_dict, edge_idx_dict, edge_attr_dict, check_nans: bool = False):
        torch.autograd.set_detect_anomaly(True)
        if check_nans:
            for node_type in x_dict:
                x = x_dict[node_type]
        ...
        for i in range(4):
            for key, value in out_dict.items():
                fc = self.fcs[i][key]
                out = fc(value)
                final_dict[key].append(out)

        # Calculate the Aggregations
        for key, value in final_dict.items():
            val = group(list(value), self.aggr)
            final_dict[key] = self.act(self.lin_2(val))
            if check_nans and torch.isnan(final_dict[key]).any():
                print(f"NAN FOUND in AGGREGATION")
...
class HeteroMinMaxNorm(nn.Module):
    ...
    def forward(self, x_dict):
        ...
        x_concat = torch.cat([x_dict[key] for key in ["SB", "PQ", "PV", "NB"]], dim=0)
        ...
        split_sizes = [x_dict[key].size(0) for key in ["SB", "PQ", "PV", "NB"]]
        x_split = torch.split(x_scaled, split_sizes, dim=0)

        out_dict = {key: x_split[idx] for idx, key in enumerate(["SB", "PQ", "PV", "NB"])}
        return out_dict
...
class ACOPFExtender(torch.nn.Module):
    def __init__(self, num_layers: int, act_fn: str = "elu"
